...
class InseqFeatureAttributionSequence(BaseModel):
            ...
            try:
                ...
                if len(attribution_matrix.shape) > 2:
                    ...
                    if len(attribution_matrix.shape) == 3:
                        # Handle NaN and Inf values - replace with zeros or finite values
                        if np.isnan(attribution_matrix).any() or np.isinf(attribution_matrix).any():
                            logger.warning(f"NaN or Inf values detected in attribution matrix, replacing with finite values")
                            attribution_matrix = np.nan_to_num(attribution_matrix, nan=0.0, posinf=1e30, neginf=-1e30)
                    ...
                    # Final check for NaN and Inf values after all transformations
                    if np.isnan(attribution_matrix).any() or np.isinf(attribution_matrix).any():
                        logger.warning(f"NaN or Inf values still present after transformation, replacing with finite values")
                        ...
                        inf_mask = np.isinf(attribution_matrix)
                        if inf_mask.any():
                            attribution_matrix = np.nan_to_num(attribution_matrix, nan=0.0, posinf=1e30, neginf=-1e30)
    ...
    def _normalize_attribution_matrix(matrix: np.ndarray) -> np.ndarray:
        ...
        if np.isnan(matrix).any() or np.isinf(matrix).any():
            logger.warning(f"Matrix contains NaN or Inf values, replacing with finite values")
